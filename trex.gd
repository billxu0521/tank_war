...
func _process(delta: float) -> void:
	...
	# 尾巴左右擺，一節比一節晚
	for i in 4:
		_pose("tail%d" % (i + 1), Vector3.UP,
			sin(_phase * 0.6 - i * 0.55) * 0.10 * (0.5 + stride))

	# 脖子和頭：走路時微點頭，咬的時候往前送
	_pose("spine1", Vector3.RIGHT, 0.06 + sin(_phase * 0.4) * 0.02)
	_pose("neck", Vector3.RIGHT, -0.20 + sin(_phase) * 0.05 * stride + chomp * 0.35)
	_pose("head", Vector3.RIGHT, 0.18 - chomp * 0.25)
